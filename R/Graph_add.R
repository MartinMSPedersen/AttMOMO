...
AttData <- data.table(read.table(file = "H:/SFSD/INFEPI/Projekter/AKTIVE/MOMO/AttMOMO/Denmark/AttMOMO_2020-W18/output/AttData_GSInfl_GSCOVID19.txt",
                                 sep = ";", header = TRUE, as.is = TRUE))

dt <- AttData[(group == 'TotalPooled'), .(ISOweek, deaths, ET, GSInfl, GSCOVID19,
                                   EB, EB_2L = EB - 2*sqrt(VEB), EB_2U = EB + 2*sqrt(VEB), EB_4U = EB + 4*sqrt(VEB),
                                   EBGSInfl = EB + EGSInfl,
