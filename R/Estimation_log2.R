...
estimation <- function(country, wdir, StartWeek, EndWeek, groups, pooled = NULL, indicators, restrict = TRUE, population = FALSE,
  ...
  # pooled <- c('00to04', '05to14', '15to64', '65to74', '75to84', '85P')
  ...
  # Post estimation ---------------------------------------------------------
  AttData[, `:=`(EET = (exp(plogET - plogB) - 1)*EB, VlogEET = ifelse(plogET == plogB, 0, VlogET + VlogB + VlogEB))]
  for (i in indicators) {
    expr <- parse(text = paste0("`:=`(E", i, " = (exp(plog", i, " - plogB) - 1)*EB,
                                VlogE", i, " = ifelse(plog", i, " == plogB, 0, Vlog", i, " + VlogB + VlogEB))"))
    AttData[, eval(expr)]
  }
  # Clean up
  AttData[, `:=`(plogB = NULL, plogET = NULL, VlogET = NULL)]
  for (i in indicators) {
    expr <- parse(text = paste0("`:=`(plog", i, " = NULL, Vlog", i, " = NULL)"))
    AttData[, eval(expr)]
  }
  ...
  if (!is.null(pooled)) {
    pooledData <- AttData[group %in% pooled,
                          .(group = 'TotalPooled',
                            ...
                            N = sum(N, na.rm = TRUE),
                            VlogB = pmax(0, sum((EB^2)*VlogB, na.rm = TRUE)/(sum(EB, na.rm = TRUE)^2)),
                            EB = sum(EB, na.rm = TRUE),
                            VEB = sum(VEB, na.rm = TRUE),
                            VlogEB = pmax(0, sum(VEB, na.rm = TRUE)/(sum(EB, na.rm = TRUE)^2), na.rm = TRUE),
                            EET = sum(EET, na.rm = TRUE),
                            VlogEET = pmax(0, sum((log(EET)^2)*VlogEET, na.rm = TRUE)/(sum(EET, na.rm = TRUE)^2), na.rm = TRUE)
                          ), keyby = .(ISOweek)]

    for (i in indicators) {
      expr <- parse(text = paste0(".(", i, " = sum(", i, "*N/sum(N), na.rm = TRUE),
                                  E", i, " = sum(E", i, ", na.rm = TRUE),
                                  VlogE", i, " = pmax(0, sum((E", i, "^2)*VlogE", i, ", na.rm = TRUE)/(sum(E", i, ", na.rm = TRUE)^2), na.rm = TRUE))"))
